...
from yolov7.utils.general import LOGGER, check_requirements, increment_path
        ...
        try:
            with zipfile.ZipFile(val_tools_target_location / 'data.zip', 'r') as zip_file:
                for member in tqdm(zip_file.namelist(), desc=f'Extracting MOT ground truth'):
                    ...
                    if os.path.exists(val_tools_target_location / member) or os.path.isfile(
                            val_tools_target_location / member):
                        pass
                    else:
            ...
            with zipfile.ZipFile((val_tools_target_location / (benchmark + '.zip')), 'r') as zip_file:
                if opt.benchmark == 'MOT16':
                    ...
                    for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                        if os.path.exists(val_tools_target_location / 'data' / 'MOT16' / member) or os.path.isfile(
                                val_tools_target_location / 'data' / 'MOT16' / member):
                            pass
                        else:
                            zip_file.extract(member, val_tools_target_location / 'data' / 'MOT16')
                else:
                    for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                        if os.path.exists(val_tools_target_location / 'data' / member) or os.path.isfile(
                                val_tools_target_location / 'data' / member):
                            pass
                        else:
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', type=str, default=WEIGHTS / 'yolov7_crowd.pt', help='model.pt path(s)')
...
def main(opt):
    ...
    MOT_results_folder = val_tools_target_location / 'data' / 'trackers' / 'mot_challenge' / Path(
        str(opt.benchmark) + '-' + str(opt.split)) / save_dir.name / 'data'
    ...
    if not opt.eval_existing:
        ...
        for i, seq_path in enumerate(seq_paths):
            ...
            p = subprocess.Popen([
                ...
                "--strong-sort-weights", opt.reid_weights,
    ...
    results = (save_dir.parent / opt.eval_existing / 'tracks' if opt.eval_existing else save_dir / 'tracks').glob(
        '*.txt')
    for src in results:
